...
app.use(user,passwords.router,bank.router,cards.router,address.router,notes.router)
...
app.post('/masterdelete',(req,res)=>{
    const token = req.header("Auth")
    if(token){
        const verification = jwt.verify(token,secret)
        if(verification){
        address.model.deleteMany({},(error)=>{
            console.log(error)
        })
        passwords.model.deleteMany({},(error)=>{
            console.log(error)
        })
        cards.model.deleteMany({},(error)=>{
            console.log(error)
        })
        notes.notes.deleteMany({},(error)=>{
            console.log(error)
        })
        bank.bank.deleteMany({},(error)=>{
            console.log(error)
        })
        }
