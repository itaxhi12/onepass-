...
export default function CardDetails({navigation})
{
  ...
  const getData  = async ()=>{
    ...
    await axios.get('http://127.0.0.1:3000/cards',{headers:{Auth:token}}).then((res)=>{
    setData(res.data)
})  }
getData()
},[setData])
    ...
    setFilter(
      data.filter((obj) =>
        obj.name.toLowerCase().includes(search.toLowerCase()) || obj.bankname.toLowerCase().includes(search.toLowerCase())
      )
    );
